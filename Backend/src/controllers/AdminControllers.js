...
import {
  ...
  SubmitedApplicantFromQuery ,
AllApplicantFromQuery ,
ApplicantFromToDeleteQuery ,
ApplicantFromToShowQuery,
...
const createJobPost = (req, res) => { 
  const uuid = uuidv4();
  const values = [uuid, req.body.jobName, req.body.jobCategory , req.body.jobExperience , req.body.jobType, req.body.jobTime ,req.body.startSalary ,req.body.endSalary ,req.body.jobSummary];  
...
const editJobPost = (req, res) => {
  ...
  const values = [req.body.jobName, req.body.jobCategory , req.body.jobExperience , req.body.jobType, req.body.jobTime ,req.body.startSalary ,req.body.endSalary ,req.body.jobSummary];
...
//  Applicant From Router
const SubmitedApplicantFrom = (req, res) => {  
  const values = [
    uuidv4(),
    req.body.fullName,
    req.body.email,
    ...
    req.body.city,
    req.body.phone,
    // req.body.githubLink || null,
    req.file.filename, 
  ];
  db.query(SubmitedApplicantFromQuery, [values], (err, result) => {   
    if (err) return res.json({ Status: false, Error: err });
    return res.json({ Status: true, Result: result });
  });
};

const AllApplicantFrom  = (req, res) => {
  db.query(AllApplicantFromQuery , (err, result) => {
    if (err) return res.json({ Status: false, Error: err });
    return res.json({ Status: true, Result: result });
  });
};

const ApplicantFromToDelete = (req, res) => {
  const uuid = req.params.id;
  db.query(ApplicantFromToDeleteQuery, [uuid], (err, result) => {
    if (err) return res.json({ Status: false, Error: "Query Error" + err });
    return res.json({ Status: true, Result: result });
  });
};

const ApplicantFromToShow = (req, res) => {
  const uuid = req.params.id;
  db.query(ApplicantFromToShowQuery, [uuid], (err, result) => {
    if (err) return res.json({ Status: false, Error: "Query Error" + err });
    return res.json({ Status: true, Result: result });
  });
};
