...
//job router query   
const createJobPostQuery = `INSERT INTO jobpost (uuid, jobName	, jobCategory , experience	, jobType 	, jobTime 	, salaryStart 	, salaryEnd 	, jobSummary ) VALUES ( ? )`;
...
const editJobPostQuery = `UPDATE jobpost SET jobName = ?, jobCategory = ?, experience = ?, jobType = ?, jobTime = ?, salaryStart = ?, salaryEnd = ?, jobSummary = ? WHERE uuid = ?`;
const jobPostToDeleteQuery = `DELETE FROM jobpost WHERE uuid = ?`;

// Submited Applicant  From Message Query
const SubmitedApplicantFromQuery  = `INSERT INTO apply_from (uuid, fullName, email , linkedin , portfolio , website , city , phone , resume ) VALUES ( ? )`;
const AllApplicantFromQuery  = `SELECT * FROM apply_from ORDER BY apply_from.ID DESC`;
const ApplicantFromToDeleteQuery  = `DELETE FROM apply_from WHERE uuid = ?`;
const ApplicantFromToShowQuery = `SELECT * FROM apply_from WHERE uuid = ?`;
